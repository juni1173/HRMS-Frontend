...
const CancelInterview = ({ uuid, interviewID, CallBack }) => {
    const Api = apiHelper()
    ...
    const onCancelInterview = async () => {
        if (reason_for_cancel !== '') {
            ...
            await Api.jsonPatch(`/interviews/candidate/job/cancel/${uuid}/${interviewID}/`, formData)
            .then(result => {
                if (result) {
        ...
        <form>
            <div className='row'>
                ...
                <div className='col-lg-4 mt-1'>
                    <Button className='btn btn-danger' onClick={onCancelInterview}>
                        Cancel Interview
                    </Button>
                </div>
