...
import { Card, CardBody, Spinner, Row, Col, Badge, Button } from 'reactstrap'
import apiHelper from '../../../../Helpers/ApiHelper'
import IssueDetail from '../../../../Projects/Components/JiraProjectComponents/IssueDetail'
import StatusChart from '../../../../Projects/Components/JiraProjectComponents/StatusChart'
import AssigneeChart from '../../../../Projects/Components/JiraProjectComponents/AssigneeChart'
...
const Issues = ({ data }) => {
        ...
        const getIssues = async () => {
            ...
            formData['fields'] =  [
              ...
              formData['jql'] = `project = ${data.project_code} & assignee = '${data.jira_account_id}' & status in ('To Do', 'In Progress') ORDER BY created DESC`
        ...
        <Fragment>
            <Row style={{height: '400px'}}>
                        ...
                        issues.map((item, key) => (
                            <Card key={key} onClick={() => onClickIssue(item.fields)} className='cursor-pointer'>
                                <CardBody>
                                    <Row>
                                        <Col md={12}>
                                            <Badge className='badge-glow' color='light-success'>{item.key}</Badge> 
                ...
                <Col md={3} className='right-scroll'>
                    ...
                    {Object.values(issues).length > 0 ? (
                        <>
                         <StatusChart data={convert(issues)} project={data.project_code} assignee={data.jira_account_id}/>
                         <br></br>
