...
const AttendcanceChart = ({ data }) => {
  const { colors } = useContext(ThemeColors)
    ...
    const seriesData = [
      data[0].total_presents,
      data[0].total_wfh,
      data[0].total_leaves,
      data[0].total_weekday_holidays,
      ...
      return (
        <>
            ...
            <CardBody className='d-flex justify-content-center p-0'>
                ...
                <div>
                {!hasMultipleValues ? (
                    <div style={{ textAlign: 'center', fontSize: '24px', fontWeight: 'bold', color: colors.primary.main }}>
                      
                    </div>
                ) : (
                  <Chart options={options} series={seriesData} type='donut' height={250} />
                )}
                </div>
            </CardBody>
            </Card>
