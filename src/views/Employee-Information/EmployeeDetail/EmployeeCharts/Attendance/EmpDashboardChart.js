...
const EmpDashboardChart = ({ data }) => {
  const { colors } = useContext(ThemeColors)
    ...
    //   }
      ...
      return (
        <>
            <Card className='holiday-chart align-item-left'>
            <CardBody className='d-flex justify-content-center p-0'>
                <div>
                    <Table striped responsive >
                        <thead>
                            <tr>
                                <th className='cursor-pointer' ><MdWork size={17} className='text-primary' /> <span style={{fontSize:'large'}}>{data[0].total_working_days && data[0].total_working_days}<span className='font-small-1 text-secondary'> Working Days</span></span><br></br><span className='font-small-1 text-secondary'>{data[0].total_working_days && convertDaysToYearsMonths(data[0].total_working_days)}</span></th>
                                <th><span style={{fontSize:'x-large'}}>{(((data[0].total_presents + data[0].total_wfh + data[0].total_leaves + data[0].total_weekday_holidays) / data[0].total_working_days) * 100).toFixed('2') > 100 ? '100' : (((data[0].total_presents + data[0].total_wfh + data[0].total_leaves + data[0].total_weekday_holidays) / data[0].total_working_days) * 100).toFixed('2')}%</span><br></br><span className='font-small-1 text-secondary'> Including Leaves</span></th>
                            </tr>
                            <tr>
...
export default EmpDashboardChart