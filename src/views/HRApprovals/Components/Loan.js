...
const Loan = ({ status_choices, yearoptions }) => {
    ...
    const preDataApi = async () => {
        ...
        const response = await Api.jsonPost('/reimbursements/employee/requests/loan/data/', formData)
        if (response.status === 200) {
            setData(response.data)
        } else {
            return Api.Toast('error', 'Loan data not found')
        }
    }
    const CallBack = () => {
        preDataApi()
        ...
        MySwal.fire({
            ...
            if (result.value) {
                ...
                if (comment !== '') formData['decision_reason'] = comment
                 Api.jsonPatch(`/reimbursements/update/loan/status/${id}/`, formData)
                    .then((result) => {
                            ...
                            <CardBody>
                                <div className="row">
                                    ...
                                    <div className="col-md-4">
                                        ...
                                        <span style={{color: "black", fontWeight:"10px", padding:"0.3rem 0.5rem"}}>{`${item.loan_type_title && item.loan_type_title} / ${item.purpose_of_loan_title && item.purpose_of_loan_title}`}</span>
                                    </div>
                                    <div className="col-md-4">
                                    <div className="mb-1">
                                    ...
                                    <Badge color='light-success'>
                                        ...
                                        <span style={{color: "black", fontWeight:"10px", padding:"0.3rem 0.5rem"}}>{item.number_of_loan_installment && item.number_of_loan_installment}</span>
