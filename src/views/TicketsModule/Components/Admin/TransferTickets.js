...
const TransferTickets = () => {
  ...
  const [canvasPlacement, setCanvasPlacement] = useState('end')
    ...
    const getTicketsData = async () => {
        setLoading(true)
        await Api.get(`/ticket/transfer/to/employee/`).then(result => {
            if (result) {
        ...
        MySwal.fire({
            ...
            if (result.value) {
                ...
                if (comment !== '') {
                    ...
                    Api.jsonPatch(`/ticket/action/by/transfer/to/employee/${id}/`, formData)
                    .then((result) => {
                        if (result.status === 200) {
                            MySwal.fire({
                                icon: 'success',
  ...
  return (
    ...
    {!loading ? (
            data && Object.values(data).length > 0 ? (
                ...
                <div className="text-center">No Transferred Ticket Data Found!</div>
            )
